...
Báo cáo này trình bày quá trình khảo sát, phân tích yêu cầu, đặc tả use case, thiết kế kiến trúc, thiết kế cơ sở dữ liệu, giao diện, kiểm thử và định hướng phát triển cho đề tài \textbf{\topic}. Hệ thống được thiết kế theo mô hình web full-stack, gồm frontend React/Vite, backend Spring Boot, cơ sở dữ liệu PostgreSQL, xác thực JWT/Google OAuth, gửi email OTP, tích hợp thanh toán qua SePay và cơ chế thông báo cho cư dân/quản trị viên.

Mục tiêu của báo cáo không chỉ là mô tả sản phẩm cuối cùng mà còn thể hiện tư duy kỹ nghệ phần mềm: xác định bài toán nghiệp vụ, phân rã tác nhân, mô hình hóa luồng xử lý, thiết kế dữ liệu có tính toàn vẹn, xây dựng kịch bản kiểm thử và chuẩn bị tài liệu đủ rõ để nhóm có thể tiếp tục triển khai, nghiệm thu và bảo trì. Các ảnh giao diện trong báo cáo được để sẵn bằng macro placeholder; sau khi chạy ứng dụng \projectname{}, chỉ cần chụp màn hình và đặt đúng tên file tương ứng.
...
Đề tài \projectname{} được lựa chọn vì có tính thực tiễn cao và phạm vi đủ rộng để minh họa nhiều nội dung quan trọng của công nghệ phần mềm. Hệ thống hướng tới hai nhóm người dùng chính: \textbf{quản trị viên} đại diện ban quản lý và \textbf{cư dân} đại diện người dùng cuối trong chung cư. Quản trị viên có thể quản lý căn hộ, tài khoản, hóa đơn, mẫu khoản thu, chiến dịch đóng góp, thanh toán, thông báo và phản ánh. Cư dân có thể xem thông tin căn hộ, xem thành viên cùng căn hộ, theo dõi hóa đơn, tạo invoice thanh toán qua QR, gửi phản ánh, xem thông báo và tham gia các khoản đóng góp.

\section{Mục tiêu dự án}
\begin{itemize}
  \item Xây dựng ứng dụng web quản lý chung cư có giao diện rõ ràng, phân tách trang dành cho cư dân và trang quản trị.
  \item Cung cấp backend REST API bằng Spring Boot, có phân lớp controller, service, repository, DTO, entity và exception handler.
  \item Thiết kế dữ liệu quan hệ cho các nghiệp vụ: tài khoản/cư dân, căn hộ, hóa đơn, invoice, payment, khoản đóng góp, phản ánh và thông báo.
  ...
  \item Cung cấp cơ chế notification tự động để cư dân/quản trị viên nhận biết sự kiện quan trọng như tạo hóa đơn, thanh toán thành công, phản ánh mới, duyệt phản ánh và phát động chiến dịch đóng góp.
\end{itemize}
...
\begin{itemize}
  ...
  \item Webhook SePay nhận giao dịch chuyển khoản, kiểm tra nội dung chuyển khoản, số tiền, trạng thái invoice và ghi nhận payment.
  ...
  \item Quản lý thông báo tự động/thủ công, template thông báo, trạng thái đã đọc và xóa mềm.
\end{itemize}

Các chức năng ngoài phạm vi hiện tại gồm: ứng dụng mobile native, phân quyền nhiều cấp ngoài \texttt{ADMIN}/\texttt{USER}, chữ ký số, hợp đồng thuê/mua căn hộ, đặt lịch tiện ích chung, chat thời gian thực và dashboard phân tích nâng cao.
...
Nhóm chat Zalo/Facebook & Gửi thông báo nhanh, cư dân quen sử dụng. & Thông tin trôi, khó phân loại hóa đơn/phản ánh, không có lịch sử nghiệp vụ chuẩn. & Cần notification có danh mục, trạng thái đọc và liên kết tới đối tượng liên quan. \\
Chuyển khoản ngân hàng thủ công & Linh hoạt, không phụ thuộc phần mềm lớn. & Ban quản lý phải đối soát thủ công, dễ nhầm nội dung chuyển khoản. & Cần mã tham chiếu invoice và webhook để tự động ghi nhận payment. \\
...
\projectname & Tích hợp căn hộ, hóa đơn, invoice QR, webhook, đóng góp, phản ánh và thông báo trong một hệ thống. & Phiên bản hiện tại chưa có mobile app, audit log đầy đủ và báo cáo tài chính nâng cao. & Phát triển theo hướng tinh gọn, minh bạch dữ liệu và ưu tiên luồng thanh toán. \\
\bottomrule
\end{longtable}

\section{Phương pháp luận phát triển: Agile/Scrum}
Dự án phù hợp với Agile/Scrum vì nghiệp vụ chung cư có thể thay đổi theo phản hồi của người dùng: cách sinh hóa đơn, cách đặt nội dung chuyển khoản, loại phản ánh, mẫu thông báo hoặc quy định đóng góp. Nhóm có thể chia backlog thành các epic: xác thực, căn hộ/cư dân, hóa đơn, invoice/thanh toán, phản ánh, thông báo, đóng góp và dashboard. Mỗi sprint tạo ra một phần có thể chạy được: ban đầu thiết lập React/Vite và Spring Boot/PostgreSQL, sau đó triển khai đăng nhập, quản lý căn hộ, hóa đơn, thanh toán, rồi mở rộng sang phản ánh và notification.

Definition of Done cho một chức năng gồm: API có DTO hợp lệ, validation rõ ràng, phân quyền đúng, transaction bảo vệ dữ liệu, frontend có trạng thái loading/error, thông báo thao tác thành công/thất bại và có kịch bản kiểm thử tương ứng. Với các luồng có tiền như invoice/payment, Definition of Done cần thêm idempotency webhook, kiểm tra số tiền chính xác, trạng thái invoice hợp lệ và không cho thanh toán một hóa đơn bằng nhiều invoice đang mở.

\chapter{Đặc tả yêu cầu phần mềm}

\section{Tổng quan SRS}
\projectname{} là hệ thống client--server. Frontend React/Vite chịu trách nhiệm hiển thị giao diện, điều hướng role-based và gọi API qua HTTP/JSON. Backend Spring Boot cung cấp REST API, xử lý xác thực, phân quyền, nghiệp vụ và ánh xạ dữ liệu bằng JPA/Hibernate. PostgreSQL lưu dữ liệu chính. Gmail SMTP phục vụ OTP email. Google OAuth hỗ trợ đăng nhập bằng tài khoản Google đã được liên kết và xác minh. SePay cung cấp QR thanh toán và webhook giao dịch.
...
Quản trị viên & Người dùng vai trò \texttt{ADMIN}, đại diện ban quản lý. & Quản lý tài khoản, căn hộ, hóa đơn, invoice, payment, campaign, report, notification. \\
...
FR-05 & Quản lý tài khoản & Admin xem danh sách tài khoản, tạo tài khoản, cập nhật thông tin, reset mật khẩu, xóa tài khoản và gán căn hộ. \\
FR-06 & Quản lý căn hộ & Admin tạo, cập nhật, xóa căn hộ; xem tầng, diện tích, loại căn hộ, trạng thái và số cư dân. \\
FR-07 & Thành viên căn hộ & Admin và cư dân xem danh sách thành viên trong căn hộ; hệ thống tự cập nhật trạng thái căn hộ theo số cư dân còn hoạt động. \\
FR-08 & Mẫu khoản thu & Admin tạo mẫu hóa đơn với tên, mô tả và số tiền mặc định để dùng cho sinh hóa đơn hàng loạt. \\
FR-09 & Hóa đơn & Admin tạo hóa đơn lẻ hoặc sinh hóa đơn cho nhiều căn hộ từ template, cập nhật thông tin khi còn \texttt{UNPAID}/\texttt{OVERDUE}. \\
FR-10 & Công nợ cư dân & Cư dân xem hóa đơn của căn hộ mình, lọc theo trạng thái và không thấy hóa đơn đã hủy. \\
FR-11 & Invoice hóa đơn & Cư dân chọn một hoặc nhiều hóa đơn cùng căn hộ để tạo invoice, hệ thống tính tổng, sinh invoice code, reference code, QR URL và khóa bill bằng \texttt{invoice\_id}. \\
FR-12 & Invoice đóng góp & Cư dân tạo invoice cho một khoản đóng góp của căn hộ nếu campaign đang \texttt{ACTIVE}, còn trong thời hạn và chưa có invoice \texttt{PENDING}. \\
FR-13 & Thanh toán tự động & Webhook SePay xử lý giao dịch vào, trích xuất \texttt{PAY... INV...}, đối chiếu invoice, số tiền, trạng thái và ghi nhận payment. \\
FR-14 & Thanh toán thủ công & Admin có thể đánh dấu một nhóm bill là đã thanh toán, hệ thống tạo invoice/payment thủ công và cập nhật trạng thái bill. \\
FR-15 & Hết hạn/hủy invoice & Invoice quá hạn được job định kỳ chuyển sang \texttt{EXPIRED}; người tạo có thể hủy invoice \texttt{PENDING}; bill được giải phóng nếu invoice không còn hiệu lực. \\
FR-16 & Chiến dịch đóng góp & Admin tạo campaign dạng bắt buộc/tự nguyện, phát động, hoàn tất hoặc hủy; khi phát động sinh bản ghi contribution cho từng căn hộ. \\
FR-17 & Phản ánh cư dân & Cư dân tạo, sửa, xóa phản ánh khi còn \texttt{PENDING}; admin xem danh sách, lọc và duyệt \texttt{APPROVED}/\texttt{REJECTED}. \\
FR-18 & Thông báo & Hệ thống tạo thông báo tự động cho bill, payment, report, campaign; admin tạo template và gửi thông báo thủ công. \\
FR-19 & Notification bell & Frontend hiển thị số thông báo chưa đọc, danh sách dropdown, đánh dấu đã đọc và điều hướng tới trang chi tiết. \\
FR-20 & Dashboard admin & Admin xem lối tắt tới các phân hệ quan trọng: căn hộ, hóa đơn, thanh toán, phản ánh, thông báo và campaign. \\
FR-21 & Dashboard cư dân & Cư dân xem tổng quan căn hộ, hóa đơn, invoice, phản ánh và đóng góp của mình. \\
FR-22 & Giao diện & Hệ thống hỗ trợ layout dashboard, sidebar, dark/light theme, toast, modal và skeleton loading. \\
\bottomrule
\end{longtable}
...
UC-03 & Cập nhật hồ sơ & Cư dân/Admin & Thay đổi thông tin cá nhân và mật khẩu. \\
UC-04 & Quản lý tài khoản & Admin & Tạo, sửa, xóa, reset mật khẩu, gán vai trò và căn hộ. \\
UC-05 & Quản lý căn hộ & Admin & Tạo/sửa/xóa căn hộ, xem chi tiết và danh sách thành viên. \\
UC-06 & Xem căn hộ của tôi & Cư dân & Xem thông tin căn hộ và thành viên cùng căn hộ. \\
UC-07 & Quản lý mẫu khoản thu & Admin & Tạo/sửa/xóa bill template. \\
UC-08 & Tạo và sinh hóa đơn & Admin & Tạo bill lẻ hoặc sinh hàng loạt cho nhiều căn hộ. \\
UC-09 & Xem hóa đơn & Cư dân/Admin & Cư dân xem bill căn hộ; admin xem toàn bộ bill. \\
UC-10 & Tạo invoice thanh toán & Cư dân & Chọn bill/contribution và nhận QR thanh toán. \\
UC-11 & Xử lý webhook thanh toán & SePay & Gửi giao dịch vào, hệ thống ghi payment và cập nhật invoice/bill. \\
UC-12 & Quản lý chiến dịch đóng góp & Admin & Tạo, phát động, hoàn tất hoặc hủy campaign. \\
UC-13 & Đóng góp theo campaign & Cư dân & Xem khoản đóng góp của căn hộ, tạo invoice và thanh toán. \\
UC-14 & Gửi phản ánh & Cư dân & Tạo/sửa/xóa phản ánh chờ xử lý. \\
UC-15 & Duyệt phản ánh & Admin & Xem danh sách phản ánh và phản hồi kết quả duyệt. \\
UC-16 & Quản lý thông báo & Admin & Tạo template, gửi thông báo thủ công, xóa mềm thông báo. \\
UC-17 & Xem thông báo & Cư dân/Admin & Xem, lọc, đánh dấu đã đọc và xóa thông báo. \\
UC-18 & Theo dõi thanh toán & Admin & Xem danh sách invoice/payment và chi tiết từng giao dịch. \\
\bottomrule
\end{longtable}

\section{Yêu cầu phi chức năng}
\subsection{Hiệu năng}
Các màn hình danh sách như tài khoản, căn hộ, phản ánh và thông báo cần hỗ trợ lọc, tìm kiếm hoặc phân trang để tránh tải quá nhiều dữ liệu. Backend sử dụng repository query/projection cho các tổng hợp như số cư dân từng căn hộ và công nợ từng căn hộ nhằm giảm nguy cơ N+1 query. Các thao tác tài chính cần chạy trong transaction để giữ nhất quán giữa invoice, bill snapshot, payment và trạng thái bill.

\subsection{Bảo mật}
Backend sử dụng Spring Security, JWT stateless session, BCrypt cho mật khẩu và phân quyền theo \texttt{ROLE\_ADMIN}/\texttt{ROLE\_USER}. Các endpoint nhạy cảm yêu cầu token hợp lệ; nhiều API kiểm tra thêm quyền sở hữu dữ liệu, ví dụ cư dân chỉ xem invoice do mình tạo hoặc bill thuộc căn hộ mình. Webhook thanh toán không dùng JWT mà dùng API key qua header để phù hợp tích hợp bên ngoài. Google OAuth phải xác minh ID token và email đã verified. Luồng quên mật khẩu dùng OTP và reset token có thời hạn.

\subsection{Độ tin cậy và toàn vẹn dữ liệu}
Các thao tác tạo invoice phải khóa hóa đơn bằng \texttt{invoice\_id} để tránh một bill được thanh toán song song trong nhiều invoice. Khi invoice hủy/hết hạn, bill được giải phóng nếu vẫn khóa bởi invoice đó. Khi thanh toán thành công, invoice chuyển \texttt{PAID}; nếu invoice dạng bill, các bill snapshot được đánh dấu \texttt{PAID}; nếu invoice dạng contribution, hệ thống tính lại số tiền đã đóng của căn hộ bằng tổng payment thành công. Webhook phải idempotent bằng \texttt{transactionId}.

\subsection{Khả dụng và trải nghiệm người dùng}
Frontend cần có điều hướng rõ ràng theo vai trò. Người dùng \texttt{ADMIN} được chuyển tới \texttt{/admin-panel}; người dùng \texttt{USER} được chuyển tới \texttt{/resident-home}. Các trang chính dùng layout dashboard, sidebar, toast, modal, skeleton loading và bảng dữ liệu. Lỗi nghiệp vụ cần hiển thị dễ hiểu: hóa đơn đã bị khóa bởi invoice khác, invoice đã hết hạn, số tiền chuyển khoản không khớp, tài khoản chưa gắn căn hộ hoặc phản ánh đã được duyệt nên không thể sửa.

\subsection{Môi trường triển khai}
Backend dùng Spring Boot 3.5, Java 25, JPA/Hibernate, PostgreSQL, Spring Mail, Spring Security, JJWT, MapStruct, Lombok, Google API Client và dotenv. Frontend dùng React 19, React Router 7, Axios và Vite. Cấu hình quan trọng được đưa vào biến môi trường: database, mail, JWT secret, thời hạn JWT, thông tin SePay và Google client ID.
...
\endhead
Mục tiêu & Người dùng đăng nhập thành công và được chuyển tới dashboard đúng vai trò. \\
...
Hậu điều kiện & Frontend lưu JWT và thông tin user trong \texttt{localStorage}; route được bảo vệ theo vai trò. \\
...
\item Frontend gửi \texttt{POST /api/auth/login} hoặc \texttt{POST /api/auth/google-login}.
\item Backend xác thực mật khẩu bằng BCrypt hoặc xác minh Google ID token.
\item Backend sinh JWT chứa username và role.
\item Frontend lưu token, lưu user và điều hướng admin tới \texttt{/admin-panel}, cư dân tới \texttt{/resident-home}.
\end{enumerate} \\
Luồng ngoại lệ E1 & Sai username/mật khẩu: backend trả lỗi xác thực, frontend hiển thị thông báo thất bại. \\
Luồng ngoại lệ E2 & Google token hết hạn/không hợp lệ hoặc email chưa verified: backend từ chối đăng nhập. \\
Luồng ngoại lệ E3 & Vai trò không khớp route: \texttt{ProtectedRoute} chuyển người dùng về trang đăng nhập. \\
Quy tắc nghiệp vụ & Token được gửi qua header \texttt{Authorization: Bearer <token>}; backend hoạt động stateless, không lưu session phía server. \\
\bottomrule
\end{longtable}
...
\toprule Thuộc tính & Nội dung \\\midrule
\endhead
Mục tiêu & Admin quản lý danh sách căn hộ, trạng thái cư trú và người dùng thuộc từng căn hộ. \\
Actor chính & Quản trị viên. \\
Tiền điều kiện & Admin đã đăng nhập; dữ liệu căn hộ hoặc user cần thao tác tồn tại nếu là cập nhật/xóa. \\
Hậu điều kiện & Thông tin căn hộ/user được lưu; trạng thái căn hộ phản ánh số cư dân còn hoạt động. \\
Luồng chính & \begin{enumerate}
\item Admin mở trang \texttt{/apartments}.
\item Frontend gọi \texttt{GET /api/apartments} để lấy danh sách kèm số cư dân và công nợ.
\item Admin tạo hoặc cập nhật căn hộ với số căn, tầng, diện tích và loại căn hộ.
\item Admin mở chi tiết \texttt{/apartment/:apartmentId} hoặc danh sách thành viên \texttt{/apartment/:apartmentId/members}.
\item Admin tạo/cập nhật tài khoản cư dân, gán \texttt{apartmentId}, trạng thái và quan hệ với chủ hộ.
\item Backend lưu thay đổi và tự cập nhật trạng thái căn hộ nếu cần.
\end{enumerate} \\
Luồng ngoại lệ E1 & Số căn hộ đã tồn tại: backend trả lỗi trùng dữ liệu. \\
Luồng ngoại lệ E2 & Xóa căn hộ còn cư dân: backend từ chối để bảo vệ toàn vẹn dữ liệu. \\
...
\toprule Thuộc tính & Nội dung \\\midrule
\endhead
Mục tiêu & Admin tạo hóa đơn; cư dân tạo invoice QR; webhook thanh toán tự động cập nhật trạng thái. \\
Actor chính & Admin, cư dân, SePay. \\
Tiền điều kiện & Cư dân đã được gắn căn hộ; bill ở trạng thái \texttt{UNPAID} hoặc \texttt{OVERDUE}; bill chưa bị khóa bởi invoice khác. \\
Hậu điều kiện & Giao dịch hợp lệ tạo bản ghi \texttt{payments}; invoice chuyển \texttt{PAID}; bill chuyển \texttt{PAID}; thông báo được gửi. \\
Luồng chính & \begin{enumerate}
\item Admin tạo bill lẻ hoặc sinh hàng loạt từ \texttt{bill\_templates}.
\item Hệ thống gửi thông báo \texttt{BILL\_CREATED} cho cư dân của căn hộ.
\item Cư dân mở \texttt{/my-bills}, chọn một hoặc nhiều bill cùng căn hộ và gửi \texttt{POST /api/invoices/bill}.
\item Backend validate bill, tính tổng tiền, sinh \texttt{invoiceCode}, \texttt{referenceCode}, \texttt{qrCodeUrl}, lưu invoice và snapshot bill.
\item Cư dân quét QR/chuyển khoản với nội dung chứa \texttt{PAY... INV...}.
\item SePay gọi \texttt{POST /api/payment-webhooks}.
\item Backend kiểm tra API key, transaction id, nội dung chuyển khoản, trạng thái invoice và số tiền.
\item Nếu hợp lệ, backend tạo payment \texttt{SUCCESS}, gọi \texttt{markAsPaid}, cập nhật invoice/bill và gửi thông báo.
\end{enumerate} \\
Luồng ngoại lệ E1 & Bill không cùng căn hộ hoặc đã thanh toán: không tạo invoice. \\
Luồng ngoại lệ E2 & Invoice hết hạn/hủy/đã trả: payment ghi \texttt{FAILED} với reason tương ứng. \\
Luồng ngoại lệ E3 & Số tiền chuyển khoản thấp/cao hơn invoice: payment ghi \texttt{FAILED}. \\
Luồng ngoại lệ E4 & Webhook trùng transaction id: hệ thống trả bản ghi cũ, tránh xử lý hai lần. \\
Quy tắc nghiệp vụ & Nội dung chuyển khoản cần khớp biểu thức \texttt{PAY[A-Z0-9]+ INV[0-9]+}; invoice \texttt{PENDING} quá hạn được job định kỳ chuyển \texttt{EXPIRED} và giải phóng bill. \\
\bottomrule
\end{longtable}
...
Actor chính & Admin, cư dân. \\
Tiền điều kiện & Campaign ở \texttt{DRAFT} khi tạo/sửa; chỉ campaign \texttt{ACTIVE} trong thời hạn mới nhận đóng góp. \\
Hậu điều kiện & Khi phát động campaign, mỗi căn hộ có một bản ghi \texttt{apartment\_contributions}; payment thành công cập nhật số tiền đã đóng. \\
Luồng chính & \begin{enumerate}
\item Admin tạo campaign với tiêu đề, mô tả, loại \texttt{MANDATORY}/\texttt{VOLUNTARY}, ngày bắt đầu/kết thúc và số tiền mục tiêu/bắt buộc.
\item Admin phát động campaign; backend tạo contribution cho toàn bộ căn hộ và gửi thông báo \texttt{CAMPAIGN\_LAUNCHED}.
...
\item Payment thành công khiến hệ thống tính lại \texttt{collectedAmount} và trạng thái contribution.
\end{enumerate} \\
Luồng ngoại lệ E1 & Campaign bắt buộc thiếu \texttt{requiredAmount}: backend từ chối tạo/sửa. \\
Luồng ngoại lệ E2 & Campaign tự nguyện lại có \texttt{requiredAmount}: backend từ chối. \\
Luồng ngoại lệ E3 & Cư dân tạo invoice cho căn hộ khác: backend từ chối theo ownership. \\
Quy tắc nghiệp vụ & Campaign chỉ được cập nhật khi còn \texttt{DRAFT}; khi \texttt{COMPLETED}, hệ thống không nhận invoice đóng góp mới nhưng payment của invoice đang pending vẫn theo hạn riêng của invoice. \\
\bottomrule
\end{longtable}
...
\toprule Thuộc tính & Nội dung \\\midrule
\endhead
Mục tiêu & Cư dân gửi phản ánh; admin xử lý; hệ thống thông báo kết quả. \\
Actor chính & Cư dân, admin. \\
Tiền điều kiện & Cư dân/admin đã đăng nhập; phản ánh tồn tại nếu xem/sửa/duyệt. \\
Hậu điều kiện & Phản ánh chuyển trạng thái phù hợp; người liên quan nhận notification. \\
Luồng chính & \begin{enumerate}
\item Cư dân mở \texttt{/reports}, tạo phản ánh gồm tiêu đề và nội dung.
\item Backend lưu report \texttt{PENDING}, gửi notification cho admin.
\item Admin mở \texttt{/admin-reports}, lọc/xem chi tiết phản ánh.
\item Admin duyệt bằng \texttt{PATCH /api/reports/\{id\}/review} với trạng thái \texttt{APPROVED} hoặc \texttt{REJECTED}.
\item Backend lưu \texttt{reviewedBy}, \texttt{reviewedAt}, \texttt{reviewNote} và gửi notification cho cư dân.
\end{enumerate} \\
Luồng ngoại lệ E1 & Cư dân sửa/xóa phản ánh đã duyệt: backend từ chối. \\
Luồng ngoại lệ E2 & Cư dân xem phản ánh của người khác: backend từ chối hoặc trả không tìm thấy. \\
Luồng ngoại lệ E3 & Admin duyệt lại phản ánh không còn \texttt{PENDING}: backend từ chối. \\
Quy tắc nghiệp vụ & Notification hỗ trợ trạng thái đọc/xóa mềm riêng cho user và admin, có \texttt{referenceType}/\texttt{referenceId} để điều hướng tới đối tượng liên quan. \\
\bottomrule
\end{longtable}
...
\node[box, right=of browser] (api) {Backend API\\Spring Boot\\JWT Security};
\node[svc, right=of api] (service) {Service Layer\\Business Rules};
...
\draw[rel] (api) -- node[lbl,above]{service call} (service);
...
Backend được tổ chức theo các package chính: \texttt{controller} nhận request; \texttt{service} chứa nghiệp vụ; \texttt{repository} truy vấn dữ liệu; \texttt{entity} ánh xạ bảng; \texttt{dtos} mô tả request/response; \texttt{enums} chuẩn hóa trạng thái; \texttt{security} xử lý JWT; \texttt{config} cấu hình bảo mật, SePay, Google và exception. Frontend được tổ chức theo \texttt{components}, \texttt{pages}, \texttt{context}, \texttt{utils} và layout dashboard. File \texttt{App.jsx} định nghĩa các route bảo vệ theo vai trò, ví dụ \texttt{/admin-panel}, \texttt{/resident-home}, \texttt{/apartments}, \texttt{/my-bills}, \texttt{/admin-invoices}, \texttt{/my-contributions}.
...
\node[actor] (admin) at (8.3,2.9) {Admin};
...
\draw[msg] (0,-1.0) -- node[mlabel,above]{Chọn bill} (3.0,-1.0);
...
\draw[msg] (9.7,-3.1) -- node[mlabel,above]{validate bill\\save invoice} (13.0,-3.1);
...
\draw[msg] (9.7,-8.7) -- node[mlabel,above,text width=3.2cm]{payment success\\invoice/bills PAID} (13.0,-8.7);
\end{tikzpicture}}
...
\section{Mô tả ERD}
Cơ sở dữ liệu \projectname{} xoay quanh hai thực thể nền tảng: \texttt{users} và \texttt{apartments}. Bảng \texttt{users} vừa lưu thông tin tài khoản vừa lưu thông tin cư dân cho vai trò \texttt{USER}, liên kết nhiều-một tới \texttt{apartments}. Mỗi căn hộ có nhiều \texttt{bills}; bill có thể được khóa vào một invoice bằng trường \texttt{invoice\_id}. Invoice lưu mã thanh toán, mã tham chiếu, tổng tiền, QR URL, trạng thái và người tạo. Bảng \texttt{invoice\_bill\_snapshots} lưu lịch sử bill thuộc invoice tại thời điểm tạo, giúp truy vết ngay cả khi bill thay đổi về sau. Payment liên kết invoice và lưu kết quả webhook/thanh toán thủ công.

Phân hệ đóng góp gồm \texttt{contribution\_campaigns} và \texttt{apartment\_contributions}. Khi campaign được phát động, hệ thống tạo một bản ghi contribution cho mỗi căn hộ. Phân hệ giao tiếp gồm \texttt{reports}, \texttt{notifications} và \texttt{notification\_templates}. Ngoài ra còn có \texttt{otp\_verification\_tokens} và \texttt{password\_reset\_tokens} phục vụ xác minh email và khôi phục mật khẩu.
...
\texttt{full\_name} & VARCHAR(255) & & NULLABLE & Họ tên cư dân hoặc admin. \\
...
\texttt{default\_amount} & NUMERIC & & NOT NULL & Số tiền mặc định khi sinh bill. \\
...
\texttt{note} & VARCHAR(255) & & NULLABLE & Ghi chú nghiệp vụ của admin. \\
\texttt{invoice\_id} & BIGINT & & NULLABLE & ID invoice đang khóa hóa đơn; null nghĩa là bill còn có thể tạo invoice mới. \\
...
\texttt{invoice\_type} & VARCHAR(20) & & NOT NULL, DEFAULT \texttt{BILL} & Loại invoice: thanh toán bill hoặc đóng góp. \\
...
\texttt{bill\_id} & BIGINT & & NOT NULL & ID bill được đưa vào invoice. \\
\bottomrule
\end{longtable}
...
\texttt{transaction\_time} & TIMESTAMP & & NOT NULL & Thời điểm giao dịch theo webhook hoặc thao tác admin. \\
\texttt{created\_at} & TIMESTAMP & & NOT NULL, auto generated & Thời điểm lưu payment vào hệ thống. \\
\bottomrule
\end{longtable}
...
\texttt{required\_amount} & NUMERIC(12,2) & & NULLABLE & Số tiền bắt buộc mỗi căn hộ cần đóng nếu campaign bắt buộc. \\
\texttt{status} & VARCHAR(20) & & NOT NULL, DEFAULT \texttt{DRAFT} & Trạng thái campaign. \\
\texttt{created\_by} & BIGINT & FK & NOT NULL, REFERENCES \texttt{users(id)} & Admin tạo campaign. \\
\texttt{created\_at} & TIMESTAMP & & NOT NULL, auto generated & Thời điểm tạo campaign. \\
\bottomrule
\end{longtable}
...
\texttt{campaign\_id} & BIGINT & FK & NOT NULL, REFERENCES \texttt{contribution\_}\newline\texttt{campaigns(id)} & Campaign liên quan. \\
\texttt{apartment\_id} & BIGINT & FK & NOT NULL, REFERENCES \texttt{apartments(id)} & Căn hộ tham gia campaign. \\
\texttt{collected\_amount} & NUMERIC(12,2) & & NOT NULL, DEFAULT 0 & Số tiền căn hộ đã đóng, được tính lại từ payment thành công. \\
...
\texttt{review\_note} & VARCHAR(255) & & NULLABLE & Ghi chú duyệt từ admin. \\
\texttt{created\_at} & TIMESTAMP & & NOT NULL, auto generated & Thời điểm cư dân tạo phản ánh. \\
\texttt{reviewed\_at} & TIMESTAMP & & NULLABLE & Thời điểm admin duyệt phản ánh. \\
\texttt{created\_by} & BIGINT & FK & NOT NULL, REFERENCES \texttt{users(id)} & Cư dân tạo phản ánh. \\
\texttt{reviewed\_by} & BIGINT & FK & NULLABLE, REFERENCES \texttt{users(id)} & Admin xử lý phản ánh. \\
\bottomrule
\end{longtable}
...
\texttt{type} & VARCHAR(255) & & NOT NULL & Loại thông báo: bill, payment, report, campaign... \\
...
\texttt{is\_deleted\_by\_admin} & BOOLEAN & & NOT NULL, DEFAULT false & Cờ xóa mềm phía admin. \\
\texttt{admin\_deleted\_at} & TIMESTAMP & & NULLABLE & Thời điểm admin xóa mềm. \\
\texttt{reference\_type} & VARCHAR(255) & & NOT NULL & Loại đối tượng liên quan: bill, invoice, payment, report, campaign... \\
\texttt{reference\_id} & BIGINT & & NULLABLE & ID đối tượng liên quan để frontend điều hướng. \\
...
\toprule Trường & Kiểu dữ liệu & Khóa & Ràng buộc & Mô tả \\\midrule
\endhead
\texttt{id} & BIGINT & PK & AUTO INCREMENT & Định danh template thông báo. \\
\texttt{title} & VARCHAR(255) & & NOT NULL & Tiêu đề mẫu thông báo. \\
\texttt{message} & TEXT & & NOT NULL & Nội dung mẫu thông báo. \\
\texttt{created\_at} & TIMESTAMP & & NOT NULL, auto generated & Thời điểm tạo template. \\
\texttt{updated\_at} & TIMESTAMP & & NOT NULL, auto updated & Thời điểm cập nhật template gần nhất. \\
\bottomrule
\end{longtable}
...
Giao diện \projectname{} ưu tiên thao tác nhanh, dữ liệu dễ quét và phân tách rõ vai trò. Admin thường làm việc với bảng, bộ lọc, modal, form tạo/sửa và trang chi tiết; cư dân cần các trang đơn giản để xem thông tin căn hộ, hóa đơn, invoice, đóng góp, phản ánh và thông báo. Frontend sử dụng \texttt{DashboardLayout}, \texttt{Sidebar}, \texttt{NotificationBell}, \texttt{Toast}, \texttt{Modal}, \texttt{LoadingSkeleton} và \texttt{ThemeToggle} để giữ trải nghiệm nhất quán.

\section{Giao diện đăng nhập}
\placeholderimage{screenshots/01-login.png}{Giao diện đăng nhập BlueMoon}
\begin{table}[H]\centering\caption{Thành phần giao diện đăng nhập}\begin{tabularx}{\textwidth}{L{3.5cm} L{4cm} Y}\toprule Thành phần & Loại & Chức năng \\\midrule Logo/brand BlueMoon & Nhận diện & Hiển thị tên hệ thống quản lý chung cư. \\
Form username/password & Form & Đăng nhập bằng tài khoản nội bộ. \\
Nút Google Sign-In & Button & Đăng nhập bằng tài khoản Google đã liên kết. \\
Forgot password & Link/flow & Dẫn tới luồng OTP email và reset password. \\
...
\placeholderimage{screenshots/02-admin-dashboard.png}{Dashboard quản trị viên}
\begin{table}[H]\centering\caption{Thành phần dashboard admin}\begin{tabularx}{\textwidth}{L{3.5cm} L{4cm} Y}\toprule Thành phần & Loại & Chức năng \\\midrule Sidebar & Navigation & Điều hướng tới tài khoản, căn hộ, hóa đơn, invoice, payment, report, notification, campaign. \\
Quick actions & Button/Card & Mở nhanh các phân hệ admin thường dùng. \\
Notification bell & Icon dropdown & Hiển thị số thông báo chưa đọc và danh sách gần nhất. \\
Theme toggle & Toggle & Chuyển giao diện sáng/tối. \\\bottomrule\end{tabularx}\end{table}
...
\begin{table}[H]\centering\caption{Thành phần quản lý căn hộ/tài khoản}\begin{tabularx}{\textwidth}{L{3.5cm} L{4cm} Y}\toprule Thành phần & Loại & Chức năng \\\midrule Bảng căn hộ & Table & Hiển thị số căn, tầng, diện tích, loại, trạng thái, số cư dân và công nợ. \\
Form căn hộ & Modal/Form & Tạo hoặc cập nhật căn hộ. \\
Bảng tài khoản & Table & Lọc/tìm cư dân, email, vai trò, trạng thái xác minh và căn hộ liên kết. \\
Trang chi tiết & Detail page & Xem hồ sơ, reset mật khẩu, liên kết/hủy liên kết căn hộ. \\\bottomrule\end{tabularx}\end{table}

\section{Hóa đơn, invoice và thanh toán}
\placeholderimage{screenshots/05-admin-bills.png}{Trang admin quản lý hóa đơn}
...
\begin{table}[H]\centering\caption{Thành phần giao diện hóa đơn/thanh toán}\begin{tabularx}{\textwidth}{L{3.5cm} L{4cm} Y}\toprule Thành phần & Loại & Chức năng \\\midrule Bill templates & Form/Table & Quản lý mẫu khoản thu định kỳ. \\
Generate bills & Modal/Form & Sinh bill hàng loạt cho nhiều căn hộ từ template. \\
Bill list & Table & Lọc bill theo căn hộ, trạng thái và từ khóa. \\
Create invoice & Button/Action & Cư dân chọn bill để tạo invoice QR. \\
QR payment panel & Detail panel & Hiển thị mã invoice, mã tham chiếu, tổng tiền, QR URL và hạn thanh toán. \\\bottomrule\end{tabularx}\end{table}
...
\begin{table}[H]\centering\caption{Thành phần giao diện phản ánh/thông báo/đóng góp}\begin{tabularx}{\textwidth}{L{3.5cm} L{4cm} Y}\toprule Thành phần & Loại & Chức năng \\\midrule Report form & Form & Cư dân nhập tiêu đề/nội dung phản ánh. \\
Review report & Detail/Action & Admin duyệt phản ánh và nhập ghi chú xử lý. \\
Notification filter & Tabs/Select & Lọc thông báo theo nhóm bill, payment, report, campaign, system. \\
Campaign list & Table/Card & Admin quản lý campaign; cư dân xem khoản đóng góp của căn hộ. \\
Contribution invoice & Action & Cư dân tạo invoice đóng góp và thanh toán qua QR. \\\bottomrule\end{tabularx}\end{table}

\chapter{Kiểm thử phần mềm}

\section{Kế hoạch kiểm thử}
Mục tiêu kiểm thử là đảm bảo hệ thống đúng nghiệp vụ, không rò rỉ dữ liệu giữa các vai trò, giữ toàn vẹn dữ liệu tài chính và có trải nghiệm ổn định trên frontend. Chiến lược kiểm thử gồm unit test cho service, integration test cho controller/repository, E2E test cho luồng người dùng chính, security test cho phân quyền và test thủ công giao diện.
...
E2E & Luồng admin/cư dân & Trình duyệt mô phỏng & Người dùng hoàn thành nghiệp vụ từ đầu tới cuối. \\
Security & Endpoint phân quyền & Token thiếu/sai vai trò/sai ownership & Trả 401/403 hoặc 404 hợp lý, không lộ dữ liệu. \\
Payment & Invoice/webhook & Giao dịch đúng/sai số tiền, trùng transaction & Payment idempotent, trạng thái invoice/bill đúng. \\
...
\midrule
\endhead
TC-01 & Đăng nhập nội bộ & Username/mật khẩu đúng & 1) Mở \texttt{/login}. 2) Nhập thông tin. 3) Submit. & Nhận JWT, điều hướng đúng role. & Chưa thực hiện \\
TC-02 & Sai mật khẩu & Username đúng, mật khẩu sai & Submit form đăng nhập. & Backend trả lỗi, không lưu token. & Chưa thực hiện \\
TC-03 & Google login & Google ID token hợp lệ & Chọn Google Sign-In. & Email verified và tài khoản nội bộ verified thì đăng nhập thành công. & Chưa thực hiện \\
TC-04 & Quên mật khẩu & Email tồn tại & Yêu cầu OTP, xác minh OTP, đặt mật khẩu mới. & Reset password thành công, token cũ bị xóa. & Chưa thực hiện \\
TC-05 & Tạo căn hộ & Số căn mới, tầng, diện tích, type & Admin tạo căn hộ. & Căn hộ được lưu trạng thái \texttt{VACANT}. & Chưa thực hiện \\
TC-06 & Trùng số căn hộ & Số căn đã tồn tại & Admin tạo/cập nhật căn hộ. & Backend từ chối trùng dữ liệu. & Chưa thực hiện \\
TC-07 & Xóa căn hộ còn cư dân & apartmentId có user & Admin bấm xóa. & Backend từ chối. & Chưa thực hiện \\
TC-08 & Tạo tài khoản cư dân & Username, password, email, apartmentId & Admin tạo user. & User role \texttt{USER}, liên kết căn hộ đúng. & Chưa thực hiện \\
TC-09 & Cư dân xem căn hộ & User có apartment & Mở \texttt{/my-apartment}. & Hiển thị đúng căn hộ của user. & Chưa thực hiện \\
TC-10 & Cư dân chưa gắn căn hộ & User không có apartment & Mở \texttt{/my-bills}. & Backend/Frontend báo chưa được gắn căn hộ. & Chưa thực hiện \\
TC-11 & Tạo template bill & Tên, mô tả, số tiền & Admin lưu template. & Template xuất hiện trong danh sách. & Chưa thực hiện \\
TC-12 & Sinh bill hàng loạt & templateId, apartmentIds, dueDate & Admin generate bills. & Mỗi căn hộ có bill mới, gửi notification. & Chưa thực hiện \\
TC-13 & Cập nhật bill & Bill \texttt{UNPAID}, amount/dueDate mới & Admin cập nhật. & Bill đổi dữ liệu, cư dân nhận notification thay đổi. & Chưa thực hiện \\
TC-14 & Cập nhật bill đã paid & Bill \texttt{PAID} & Admin sửa bill. & Backend từ chối. & Chưa thực hiện \\
TC-15 & Tạo invoice bill & Danh sách bill cùng căn hộ & Cư dân tạo invoice. & Invoice \texttt{PENDING}, bill bị khóa \texttt{invoice\_id}. & Chưa thực hiện \\
TC-16 & Bill khác căn hộ & Chọn bill từ nhiều căn hộ & Tạo invoice. & Backend từ chối. & Chưa thực hiện \\
TC-17 & Hủy invoice & Invoice \texttt{PENDING} do user tạo & Cư dân hủy invoice. & Invoice \texttt{CANCELLED}, bill được giải phóng. & Chưa thực hiện \\
TC-18 & Webhook đúng & Nội dung \texttt{PAY... INV...}, đúng số tiền & Gửi webhook SePay. & Payment \texttt{SUCCESS}, invoice/bill \texttt{PAID}. & Chưa thực hiện \\
TC-19 & Webhook sai tiền & Amount nhỏ/lớn hơn total & Gửi webhook. & Payment \texttt{FAILED} với reason tương ứng. & Chưa thực hiện \\
TC-20 & Webhook trùng & Cùng transactionId & Gửi webhook hai lần. & Không tạo payment trùng, trả bản ghi đã có. & Chưa thực hiện \\
TC-21 & Campaign bắt buộc & requiredAmount hợp lệ & Admin tạo và launch campaign. & Tạo contributions cho toàn bộ căn hộ. & Chưa thực hiện \\
TC-22 & Campaign tự nguyện sai & Có requiredAmount & Admin tạo campaign. & Backend từ chối. & Chưa thực hiện \\
TC-23 & Cư dân gửi phản ánh & Title/content hợp lệ & Submit report. & Report \texttt{PENDING}, admin nhận notification. & Chưa thực hiện \\
TC-24 & Duyệt phản ánh & Report \texttt{PENDING} & Admin approve/reject. & Lưu reviewer/note, cư dân nhận notification. & Chưa thực hiện \\
TC-25 & Truy cập trái quyền & Token USER gọi API admin & Gửi request tới \texttt{/api/users}. & Backend trả 403, không trả dữ liệu. & Chưa thực hiện \\
\bottomrule
\end{longtable}
\endgroup
...
\section{Kết luận}
Báo cáo đã trình bày toàn diện đề tài \textbf{\topic} theo góc nhìn kỹ nghệ phần mềm. Hệ thống giải quyết các nghiệp vụ quan trọng trong quản lý chung cư: tài khoản/cư dân, căn hộ, hóa đơn, invoice QR, thanh toán tự động qua webhook, khoản đóng góp, phản ánh và thông báo. Kiến trúc React/Vite và Spring Boot giúp tách biệt frontend/backend, thuận lợi cho phát triển song song. Cơ sở dữ liệu quan hệ bám sát nghiệp vụ và có các ràng buộc quan trọng để bảo vệ tính nhất quán, đặc biệt trong luồng invoice/payment.

Điểm mạnh của dự án là phân hệ nghiệp vụ rõ, tích hợp thanh toán có kiểm tra idempotency, hệ thống notification tự động và giao diện dashboard theo vai trò. Các bảng đặc tả yêu cầu, use case, ERD, giao diện và test case trong báo cáo có thể dùng làm tài liệu nghiệm thu và định hướng phát triển tiếp.

\section{Hướng phát triển}
\begin{itemize}
  \item Bổ sung audit log cho thao tác tài chính, chỉnh sửa hóa đơn, duyệt phản ánh và gửi thông báo.
  \item Thêm dashboard phân tích: tổng công nợ, tỷ lệ thanh toán đúng hạn, khoản đóng góp theo campaign và biểu đồ theo tháng.
  \item Hỗ trợ xuất PDF/Excel cho hóa đơn, invoice, danh sách cư dân và báo cáo thanh toán.
  \item Tích hợp email/push notification song song với notification trong app.
  ...
  \item Hoàn thiện production hardening: rate limiting, CORS theo môi trường, HTTPS, backup database, webhook HMAC và rotation secret.
\end{itemize}
...
POST & \texttt{/api/bills/generate} & ADMIN & Sinh hóa đơn hàng loạt từ template. \\
GET & \texttt{/api/bills/me} & Authenticated & Cư dân xem bill căn hộ mình. \\
...
PATCH & \texttt{/api/reports/\{id\}/review} & ADMIN & Admin duyệt phản ánh. \\
...
\begin{itemize}
  ...
  \item \texttt{screenshots/02-admin-dashboard.png}: dashboard admin.
  ...
  \item \texttt{screenshots/05-admin-bills.png}: quản lý hóa đơn admin.
